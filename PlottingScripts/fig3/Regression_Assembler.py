...
plt.scatter(x, y, color='#1F77B4', alpha=0.9, s=40, zorder=10)
plt.plot(x, y_pred, color='#D62728', linewidth=2, alpha=0.9, zorder=5, 
         label=f'Fit: y = {slope:.2f}x + {intercept:.2f}\nR² = {r_squared:.4f}')
plt.xlabel('RI$_{Megahit}$', fontsize=24)
plt.ylabel('RI$_{MetaSpades}$', fontsize=24)
plt.text(10, 280, f'y = {slope:.2f}x {sign} {abs(intercept):.2f}\nR² = {r_squared:.4f}', fontsize=20)
...
for i in range(len(labels)):
    plt.text(x[i]+x_bias[i], y[i]+y_bias[i], labels[i], fontsize=11, zorder=8, color='grey')
